...
var VirtualList = /** @class */ (function (_super) {
    ...
    VirtualList.prototype.componentDidMount = function () {
        ...
        this._subs.push(this.props.options$.pipe(operators.tap(function (options) {
        ...
        // window resize
        this._subs.push(rxjs.fromEvent(window, 'resize').pipe(operators.withLatestFrom(this.options$), operators.skipWhile(function (_a) {
            var _ = _a[0], options = _a[1];
            return !options.resize;
        }), operators.startWith(null), operators.debounceTime(200), operators.map(function () { return _this.containerHeight$.next(virtualListElm.clientHeight); })).subscribe());
        // scroll events
        ...
        this._subs.push(this.options$.pipe(operators.filter(function (option) { return option.startIndex !== undefined; }), operators.map(function (option) { return option.startIndex * option.height; })
        ...
        var scrollDirection$ = scrollTop$.pipe(operators.map(function (scrollTop) {
    ...
    VirtualList.prototype.render = function () {
        var _this = this;
        return (React.createElement("div", { className: style$1.VirtualList, ref: this.virtualListRef, style: this.props.style },
            React.createElement("div", { className: style$1.VirtualListContainer, style: { height: this.state.scrollHeight } }, this.state.data.map(function (data, i) {
                return React.createElement("div", { key: i, className: style$1.VirtualListPlaceholder, style: { transform: "translateY(" + data.$pos + "px)" } }, data.origin !== undefined ? _this.props.children(data.origin, data.$index) : null);
            }))));
    };
...
var Vlist = /** @class */ (function (_super) {
    ...
    function Vlist(props) {
        ...
        _this.options$ = _this.state.options$.pipe(operators.throttleTime(10));
        return _this;
    }
    ...
    Vlist.prototype.componentDidMount = function () {
        var _this = this;
        this._sub.push(this.props.data$.pipe(operators.map(function (data) { return !Boolean(data.length); })).subscribe(function (isEmpty) { return _this.setState({ isEmpty: isEmpty }); }));
        this._sub.push(this.props.refresh$
            .pipe(operators.delay(1))
...
var VSelect = /** @class */ (function (_super) {
    ...
    function VSelect(props) {
        ...
        _this.state = {
            ...
            dataSource$: new rxjs.BehaviorSubject([]),
        ...
        _this.changingValue$ = new rxjs.BehaviorSubject('');
    ...
    VSelect.prototype.componentDidMount = function () {
        ...
        this.data$ = rxjs.combineLatest(this.state.dataSource$, this.changingValue$.pipe(operators.distinctUntilChanged())).pipe(operators.map(function (_a) {
    ...
    VSelect.prototype.render = function () {
        ...
        return (React.createElement(antd.Dropdown, { overlay: React.createElement(Vlist, { "data$": this.data$, children: this.props.children, keyProp: this.props.keyProp, value: this.state.realValue, itemHeight: this.props.itemHeight, index: this.state.index, emptyTpl: this.props.emptyTpl, onChange: this.onChange, "refresh$": this.refresh$ }), 
            // prevent default behavior
            trigger: [], 
            // @ts-ignore
            showAction: showAction, hideAction: ['blur'], visible: this.state.visible, onVisibleChange: this.onVisibleChange },
            React.createElement("div", { style: this.props.style, className: cls, onClick: this.onRefresh, onBlur: this.onRefresh },
                React.createElement("div", { className: "ant-select-selection ant-select-selection--single" },
                    React.createElement("div", { className: "ant-select-selection__rendered" },
                        React.createElement("div", { className: "ant-select-selection__placeholder", style: { display: isShowPlaceholder } }, this.props.placeholder),
                        React.createElement("div", { className: "ant-select-selection-selected-value", style: { display: isShowValuePlaceholder, opacity: this.state.isEdit ? 0.4 : 1 } }, this.state.value),
                        React.createElement("div", { className: "ant-select-search ant-select-search--inline", style: { display: isShowInput } },
                            React.createElement("div", { className: "ant-select-search__field__wrap" },
                                React.createElement(antd.Input, { ref: this.inputRef, className: ['ant-select-search__field', style.VSelectInput].join(' '), value: this.state.changingValue, onChange: this.onInput })))),
                    isShowClearBtn && (React.createElement("span", { className: "ant-select-selection__clear", onClick: this.clearValue },
                        React.createElement(antd.Icon, { type: "close-circle", theme: "filled", className: "ant-select-clear-icon" }))),
                    React.createElement("span", { className: ['ant-select-arrow', style.VSelectArrow].join(' ') },
